	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-sm-9 main">
				<h2>Geral todos os Dízimos e Oferta</h2>

				<table class="table table-striped">
					<tr>
						<th>NOME PESSOA</th>
						<th>DATA</th>
						<th>DÍZIMO OU OFERTA</th>
						<th>VALOR</th>
					</tr>
					<c:forEach items="${listaDizimo}" var="list">
						<tr>
							<td>${list.pessoa.nome_pessoa}</td>
							<td><fmt:formatDate
									value="${list.dizimoOferta.data_dizimo_oferta}"
									pattern="dd/MM/yyyy" /></td>
							<td><c:choose>
									<c:when test="${list.dizimoOferta.dizimo eq false}">
									OFERTA
									</c:when>
									<c:otherwise> DÍZIMO
									</c:otherwise>
								</c:choose></td>
							<td>${list.dizimoOferta.valor}</td>
						</tr>
					</c:forEach>
				</table>
			</div>
